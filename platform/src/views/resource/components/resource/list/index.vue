...
import { components, delComponents } from "@/apis/resources-components";
export default {
  components: { Pagination, AddEdit, lock, nolock },
  data() {
    return {
      filterForm: {
        name: "",
  ...
  methods: {
    ...
    operateHandle(type, row) {
      ...
      if (type === 2) {
        this.$confirm("确认删除该图标库吗, 是否继续?", "提示", {
    ...
    // 打开详情页面
    openDetails(item) {
      let fPath = "/resource_file/" + item.filePath;
      this.appendJS(fPath);
      this.$router.push({
        path: "/resource/components/resource/details/" + item.id,
        query: { name: item.name, status: item.status },
      });
    },
    //动态加载js文件
    appendJS(path) {
    ...
    getListFun() {
      let params = {
        ...
        created_end_time: this.filterForm.created_end_time,
        created_start_time: this.filterForm.created_start_time,
        page_index: this.pager.pageIndex,
        page_size: this.pager.pageSize,
        created_by: this.filterForm.created_by,
      };
      components(params).then((res) => {
        if (res.code == 200) {
