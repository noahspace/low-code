...
import { updateIcons, addIcons } from "@/apis/resources-components";
export default {
  props: ["params", "type"],
  data() {
    return {
  ...
  methods: {
    ...
    success() {
      ...
      if (this.type == "edit") {
        ...
        updateIcons(this.uploadForm).then((res) => {
          if (res.code == 200) {
        ...
        addIcons(this.uploadForm).then((res) => {
          if (res.code == 200) {
            this.$messageBox("success");
            this.$emit("close");
            this.$emit("success");
    ...
    async getGroup() {
      try {
        let res = await API.get({ type: "07", main_id: this.form.category });
        if (res.code === 200) {
          this.group = [
            { id: "-1", name: "根节点" },
